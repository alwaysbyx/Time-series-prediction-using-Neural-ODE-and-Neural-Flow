...
def add_mask(data_dict):
	...
	data_pred = data_dict["data_to_predict"]
	mask_pred = data_dict["mask_predicted_data"]

	if mask is None:
		mask = torch.ones_like(data).to(get_device(data))
		data_dict["observed_mask"] = mask
	if mask_pred is None:
		mask_pred = torch.ones_like(data_pred).to(get_device(data_pred))
		data_dict["mask_predicted_data"] = mask_pred
...
def subsample_observed_data(data_dict, n_tp_to_sample = None, n_points_to_cut = None):
	...
	if n_points_to_cut is not None:
		...
		data_pred, time_steps_pred, mask_pred = cut_out_timepoints(
			data_dict["data_to_predict"].clone(), 
			time_steps = data_dict["tp_to_predict"].clone(), 
			mask = (data_dict["mask_predicted_data"].clone() if data_dict["mask_predicted_data"] is not None else None),
			n_points_to_cut = n_points_to_cut)
	...
	if n_points_to_cut is not None:
		...
		new_data_dict["data_to_predict"] = data_pred.clone()
		new_data_dict["tp_to_predict"] = time_steps_pred.clone()
		new_data_dict["mask_predicted_data"] = mask_pred.clone()
