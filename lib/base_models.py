...
class VAE_Baseline(nn.Module):
	...
	def compute_all_losses(self, batch_dict, n_traj_samples = 1, kl_coef = 1.):
		...
		pred_y, info = self.get_reconstruction(batch_dict["tp_to_predict"], 
			batch_dict["observed_data"], batch_dict["observed_tp"], 
			mask = batch_dict["observed_mask"], n_traj_samples = n_traj_samples,
			mode = batch_dict["mode"], run_backwards=run_backwards)

		# fig = plt.figure()
		# plt.plot(batch_dict['tp_to_predict'].cpu().numpy(), batch_dict['data_to_predict'].cpu().numpy()[0].reshape(-1), 'o', color='orange', alpha=0.5, label='Extrapolate')
		# plt.plot(batch_dict['observed_tp'].cpu().numpy(), batch_dict["observed_data"].cpu().numpy()[0].reshape(-1), 'o', color='grey', alpha=0.5, label='Train')
		# plt.plot(batch_dict['tp_to_predict'].cpu().numpy(), pred_y.cpu().numpy()[0, 0, :, 0], color='blue', alpha=0.9)
		# plt.grid(axis='x', color='0.95')
		# plt.legend()
		# plt.savefig(f'results/comparison/sine',dpi=300)
